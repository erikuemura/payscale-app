    ...
    <div className="min-h-screen" style={{ fontFamily: "var(--font-geist-sans), system-ui, sans-serif" }}>
      ...
      <header className="fixed top-0 inset-x-0 z-50 h-16"
        ...
        <div className="h-full flex items-center justify-between"
          ...
          <Link href="/site" className="flex items-center gap-2 shrink-0" aria-label="PayScale Intelligence — Página inicial">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {TESTIMONIALS.map(t => (
              <div key={t.name}
                ...
                <div>
                  <div style={{ fontSize: "3rem", lineHeight: 1, color: "#2563eb", opacity: 0.15, fontFamily: "Georgia, serif", marginBottom: "-0.5rem" }} aria-hidden="true">"</div>
                  <p style={{ fontSize: "0.9375rem", color: "#1e293b", lineHeight: 1.75 }}>{t.quote}</p>
                </div>
                <div>
                  <div style={{ display: "flex", gap: "0.125rem", marginBottom: "0.75rem" }} aria-label={`${t.stars} estrelas`}>
                  ...
                  <ul style={{ display: "flex", flexDirection: "column", gap: "0.875rem" }}>
                    {p.features.map(f => (
                      <li key={f} style={{ display: "flex", alignItems: "flex-start", gap: "0.625rem", fontSize: "0.875rem", color: "#4a5568" }}>
                        <CheckCircle size={14} style={{ color: "#059669", flexShrink: 0, marginTop: "0.1875rem" }} aria-hidden="true" />
                        {f}
                      </li>
                    ))}
                  </ul>
